...
pub const Project = struct {
    ...
    pub fn extensionLib(proj: Project) *Step.Compile {
        const lib = proj.pgbuild.addExtensionLib(.{
            ...
            .source_file = proj.build.path(proj.config.source_file.?),
        });
...
pub const RunExec = struct {
    ...
    fn create(b: *Build, name: []const u8, argv: []const []const u8) *RunExec {
        ...
        r.argv.appendSlice(r.owner.std_build.allocator, argv) catch @panic("OOM");
        return r;
    }
...
pub fn addExtensionLib(b: *Build, options: ExtensionLibOptions) *Step.Compile {
    ...
    const lib = b.std_build.addLibrary(.{
        ...
        .root_module = b.std_build.createModule(.{
            .root_source_file = resolveLazyPath(b, root_dir, options.source_file, "main.zig"),
...
pub fn addRunTests(b: *Build, options: RunTestsOptions) *RunExec {
    // Use the install prefix's lib directory for loading the test extension.
    // When pg_config points to a different location (e.g., nix store), we still
    // want to load from the install prefix where the extension was installed.
    const target_lib_dir = b.std_build.pathJoin(&[_][]const u8{ b.std_build.install_prefix, "lib" });
...
pub fn runPGConfig(b: *Build, question: []const u8) []const u8 {
    ...
    if (b.debug.pg_config) {
        const joined_argv = std.mem.join(allocator, " ", argv[0..]) catch @panic("OOM");
